...
### 처음 쓰는 사용자(앱 사용 기준)
1. `/app` 접속 후 왼쪽 게시판에서 원하는 게시판을 선택합니다.
2. 글 제목을 눌러 상세를 보고, 필요 시 댓글/멘션(`@닉네임`)을 사용합니다.
3. 글 작성은 선택한 게시판에서만 가능하며, `전체 게시글`에서는 작성할 수 없습니다.
4. `근무일정` 게시판은 캘린더가 함께 표시되며, 날짜 클릭 시 `풀타임/파트1/파트2/파트3/교육`이 분리되어 보입니다.
5. 내 정보의 `모바일 알림`에서 게시판별 푸시와 `근무일정 전날/당일 푸시`를 켤 수 있습니다.

### 근무일정 알림 시간(Asia/Seoul)
- 전날 알림: 21:00
- 당일 알림: 08:30
- 조건: 근무표에 본인 실명이 매칭된 경우에만 발송

### 알림 설정 상세 절차(사용자용)
1. `/app` 왼쪽 내 정보에서 `알림 센터`를 엽니다.
2. `댓글 알림 설정`에서 `댓글 알림`, `멘션 알림`이 켜짐인지 확인합니다.
3. `게시판 알림 설정`에서 알림 받을 게시판을 켭니다.
4. 내 정보의 `모바일 알림`을 엽니다.
5. `모바일 알림 켜기`를 누르고 브라우저 권한 팝업을 `허용`합니다.
6. 상단 상태가 `기기 지원=지원됨 / 알림 권한=허용 / 활성 기기=1대 이상`인지 확인합니다.
7. `게시판별 모바일 알림`에서 받을 게시판만 켭니다.
8. `근무일정 푸시 알림`에서 `근무일정 전날/당일 푸시`를 켭니다.
9. `상태 새로고침`으로 최종 반영 상태를 확인합니다.

### 알림 설정 실패 시 체크
- iOS: 홈 화면에 추가한 아이콘(PWA)으로 실행했는지 확인
- iOS/Android 공통: 브라우저 권한, OS 알림 권한, 배터리 최적화 예외 확인
- 근무일정 푸시: 실명 매칭 여부 + 근무일정 토글 + 모바일 알림 상태 3개 확인

### 앱 내 사용설명서 위치
- `/app` 화면 우측 상단의 `사용 설명서` 버튼
...
루트 저장소(`/Users/jaegwan/Desktop/VSCODE/mentor-forum`) 기준으로 아래 패턴은 Git 추적에서 제외합니다.

- `.DS_Store`
- `mentor-forum-react/firestore-debug.log`
- `mentor-forum-react/test-results/`
- `mentor-forum-react/tmp-*.png`

정리 목적:
- 로컬 디버그 산출물/스크린샷으로 인한 커밋 노이즈를 차단
- 협업 시 불필요한 충돌 최소화
