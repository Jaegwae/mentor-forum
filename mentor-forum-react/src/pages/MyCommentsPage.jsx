...
export default function MyCommentsPage() {
  ...
  const hydrateCommentRows = useCallback(async (commentRows, boardMap) => {
    ...
    return commentRows
      ...
      .sort((a, b) => toActivityMillis(b.createdAt) - toActivityMillis(a.createdAt));
  }, []);
  ...
  const loadMyComments = useCallback(async (uid, options = {}) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: toUserErrorMessage(err, '내 댓글 목록을 불러오지 못했습니다.') });
      if (!append) {
        setComments([]);
  ...
  const excelComments = useMemo(() => {
    return comments.map((comment, idx) => ({
      ...
      dateText: formatActivityDate(comment.createdAt)
    }));
  }, [comments]);
                ...
                {loading ? (
                  ...
                  return (
                    <tr key={`${comment.postId}-${comment.id}`} className="my-activity-row" onClick={() => moveCommentPost(comment)}>
                      ...
                      <td>{formatActivityDate(comment.createdAt)}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            {!loading ? comments.map((comment, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="my-activity-mobile-meta">{formatActivityDate(comment.createdAt)}</p>
                </button>
              );
            }) : null}
            {!loading && comments.length > 0 ? (
